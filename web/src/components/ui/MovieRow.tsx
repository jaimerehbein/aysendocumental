                ...
                <div className="flex gap-4 overflow-x-auto pb-8 scrollbar-hide snap-x px-1">
                    {movies.map((movie) => (
                        <Link
                            ...
                            href={`/watch/${movie.id}`}
                            className={clsx(
                                "relative flex-none rounded-md overflow-hidden transition-all duration-300 ease-out cursor-pointer snap-start hover:z-20 hover:scale-105 hover:shadow-[0_0_20px_rgba(0,43,73,0.6)] hover:ring-2 hover:ring-max-accent",
                                cardClass
                            )}
                        >
                            <Image
